...
/**
 * @description
 * get object(database/collection) and name, and query the document/collection
 * @param object
 * @param name
 * @param {Object=} options
 * @returns {Promise}
 */
function getByName(object, name, options) {
  var method = _.isDataBase(object) ?  'queryCollections' : 'queryDocuments';
  return new Promise(function(resolve, reject) {
    clientDB[method](object._self, 'SELECT * FROM root r WHERE r.id="' + name + '"', options)
      .toArray(function(err, results) {
        return err
          ? reject(new Error(err))
          : resolve(_.isArray(results) ? results[0] : results);
...
function findOrCreateCollection(db, name, options) {
  return getByName(db, name, options)
    .then(function(data) {
      return _.isEmpty(data)
  ...
  return find(col, params, options)
    .then(function(data) {
      return _.isEmpty(data)
        ...
        : remove(data[0]); //TODO: if it's an array of results, remove them all ?
    });
}
